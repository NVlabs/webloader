...
import webloader.filters as dlf
import numpy as np
...
def test_batched():
    for sample in dlf.batched(20)(source2(100)):
        ...
        assert isinstance(sample["cls"], np.ndarray)
...
def source3(n=5):
    ...
    for i in range(n):
        yield dict(__key__="{:06d}".format(i),
                   png=torch.rand(1, 2, 3),
                   cls=i % 3,
                   info=str(i))

def test_batched_torch():
    import torch
    for sample in dlf.batched(20, combine_scalars="torch")(source3(100)):
        assert isinstance(sample["png"], torch.Tensor)
        assert isinstance(sample["cls"], torch.Tensor)
        assert tuple(sample["png"].shape) == (20, 1, 2, 3)
